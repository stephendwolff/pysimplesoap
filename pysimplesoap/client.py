...
#     def __str__(self):
#         return self.encode("utf8", "ignore")
#
...
<%(soap_ns)s:Body>
    ...
    def wsdl_call(self, method, *args, **kwargs):
        ...
        if input and args:
            ...
            d = [(k, arg) for k, arg in list(zip(input.values()[0].keys()), args)]
    ...
    def wsdl_parse(self, url, debug=False, cache=False):
        ...
        def fetch(url):
            ...
            # make md5 hash of the url for caching... 
            filename = "%s.xml" % hashlib.md5(url.encode('utf-8')).hexdigest()
            if isinstance(cache, str):
        ...
        for binding in wsdl.binding:
            ...
            for operation in binding.operation:
                ...
                d['parts'] = {}
                body = operation.input('body', ns=list(soap_uris.values()), error=False)
                d['parts']['input_body'] = body and body['parts'] or None
                body = operation.output('body', ns=list(soap_uris.values()), error=False)
                d['parts']['output_body'] = body and body['parts'] or None
                header = operation.input('header', ns=list(soap_uris.values()), error=False)
                d['parts']['input_header'] = header and header['part'] or None
                headers = operation.output('header', ns=list(soap_uris.values()), error=False)
